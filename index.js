...
window.onload = function firstCountry() {
  document.getElementById("country-fullname").innerHTML =
    ubrCountriesList[0].fullName;
  document.getElementById("country-capital").innerHTML =
    ubrCountriesList[0].capital;
  document.getElementById("country-continent").innerHTML =
    ubrCountriesList[0].continent;
  document.getElementById("country-language").innerHTML =
    ubrCountriesList[0].language;
  document.getElementById("country-population").innerHTML =
    ubrCountriesList[0].population;

  document.getElementById(
    "flag"
  ).src = `/assets/flag/${ubrCountriesList[0].simpleName}.png`;
  document
    .getElementById("flag")
    .setAttribute("alt", `Bandeira de ${ubrCountriesList[0].name}`);
  document.getElementById(
    "map"
  ).src = `/assets/map/${ubrCountriesList[0].simpleName}.png`;
  document
    .getElementById("map")
    .setAttribute("alt", `Localização de ${ubrCountriesList[0].name}`);
};

// Seleciona país e envia para a section
function selectCountry() {
  const getCountry =
    ubrCountriesList[document.getElementById("selectCountry").value];
  ...
  document.getElementById(
    "flag"
  ).src = `/assets/flag/${getCountry.simpleName}.png`;
  document
    .getElementById("flag")
    .setAttribute("alt", `Bandeira de ${getCountry.name}`);
  document.getElementById(
    "map"
  ).src = `/assets/map/${getCountry.simpleName}.png`;
  document
    .getElementById("map")
    .setAttribute("alt", `Localização de ${getCountry.simpleName}`);
}
